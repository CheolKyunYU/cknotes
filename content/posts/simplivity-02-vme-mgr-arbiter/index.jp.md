...
Step 1で管理サーバーのインフラ基盤（BaseOS、NTP、DNS、NFS）を整えたら、次は管理サーバー上に中核制御基盤であるVME Manager VM（`vmemgr`）と、スプリットブレイン防止用のArbiter VMをデプロイします。

VME ManagerはHVMクラスター全体およびSimpliVity OVCを一元的にプロビジョニング・管理する仮想化コンソールであり、Arbiterは2ノード構成におけるネットワーク障害時にデータの整合性を保つ独立クォーラムデーモンです。

本記事では、ホストTUI（`hpe-vm`）コンソールを用いたVME Manager VMのパラメータ設定と、SimpliVity Arbiterパッケージの導入および通信ポートの検証手順を解説します。
...
管理サーバー上でのVME Manager VMおよびArbiter VMの配備が完了すると、管理プレーンとクォーラム監視機能が確立され、SimpliVity物理ノードのセットアップを受け入れる準備が整います。

特に2ノードSimpliVity構成において、Arbiterを外部の独立した物理サーバー上に配置することは、電源障害やネットワーク分断時のスプリットブレインを防ぐための必須条件です。TCPポート22122の開放と双方向のIPルーティングが正しく機能しているかを事前に確認しておきましょう。
