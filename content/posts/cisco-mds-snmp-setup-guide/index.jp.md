...
MDSスイッチのSNMP設定で最も頻発する誤用が、`snmp-server enable traps` で全トラップを一括有効化してしまうことです。これにより軽微なデバッグイベントまでNMSへ大量送信され、トラップストーム（Trap Storm）が発生してスイッチCPUおよび監視サーバーに不要な負荷を与えてしまいます。

実務においては、公式ガイドに準拠し、監視対象とすべき重要MIBトラップのみを厳選して有効化することが標準的な運用プラクティスです。

本記事では、SNMP v2c基本設定（Community、Host）から必須MIB通知の選別設定、コンフィグ保存・検証、およびNMS用MIBファイルの入手先までをコマンドベースで解説します。
...
Cisco MDS SANスイッチの監視設計では、全トラップを無差別に出力させるのではなく、物理ハードウェア、FCポート状態、およびファブリック変更イベントに絞って設定することが、NMSおよびスイッチCPUの過負荷を防ぎつつ重要障害を確実に捉えるための基本原則です。
