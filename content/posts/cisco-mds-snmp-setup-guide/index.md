...
MDS 스위치에서 SNMP를 구성할 때 흔히 하는 실수가 `snmp-server enable traps` 명령어로 모든 알림을 무조건 켜버리는 것입니다. 이렇게 하면 사소한 디버그성 이벤트까지 NMS로 쏟아져 들어와 트랩 폭풍(Trap Storm)이 발생하고, 스위치 CPU와 모니터링 서버에 불필요한 부하를 주게 됩니다.

따라서 현업에서는 공식 가이드를 기반으로 반드시 감시해야 하는 핵심 MIB 알림만 선별적으로 활성화하는 것이 표준적인 운영 모범 사례입니다.

이번 글에서는 SNMP v2c 기본 설정(Community, Host)부터 필수 MIB 알림 선별 활성화, 설정 저장 및 검증, 그리고 NMS 서버용 MIB 파일 다운로드 경로까지 실무 명령어 기준으로 정리했습니다.
...
Cisco MDS SAN 스위치 모니터링은 장비 전체 트랩을 무분별하게 열기보다, 위와 같이 물리 하드웨어와 FC 포트 상태, 패브릭 변경 이벤트 위주로 선별 구성하는 것이 NMS 서버와 스위치 CPU를 보호하면서도 핵심 장애를 놓치지 않는 정석입니다.
